...
def run_a_model(tpu_id, model_type, batch_size=32, max_seq_length=384, lr=2e-5, epoch=2, run_time=0):
    ...
    xargs = f"gsutil -m cp -r gs://squad_cx/albert_data/train/{tpu_id}_{model_type}_{batch_size}_{max_seq_length}_{lr}_{epoch}_{run_time} gs://squad_cx/albert_args_train_answer_models"
    os.system(xargs)
    ...
    os.system(xargs)
    xargs = f"gsutil -m cp -r {tpu_id}_{model_type}_{batch_size}_{max_seq_length}_{lr}_{epoch}_{run_time} gs://squad_cx/albert_args_train_answer_results/"
    os.system(xargs)
