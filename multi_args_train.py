...
def run_a_model(tpu_id, model_name, model_size, batch_size, max_seq_length, lr, epoch, run_time, is_atrlp=False):
    ...
    xargs = f"gsutil -m cp -r gs://squad_cx/electra_data{tpu_id}/models/electra_large/results/squad_qa gs://squad_cx/args_train_results/{tpu_id}_{model_name}_{batch_size}_{max_seq_length}_{lr}_{epoch}_{run_time}"
    os.system(xargs)
